 ...
 */
namespace AHT\AddAttributeCategory\Block\Html;
 ...
 * @since 100.0.2
 */
class CustomTopmenu extends \Magento\Theme\Block\Html\Topmenu
{
    /**
     * Cache identities
     *
    ...
    public function getImage($entity_id) {
        $connection = $this->resource->getConnection();
        $sql = "SELECT cce.*, ccev.value, ea.attribute_code FROM `catalog_category_entity` cce
        LEFT JOIN `catalog_category_entity_varchar` ccev ON cce.entity_id = ccev.entity_id 
        LEFT JOIN `catalog_category_entity_int` ccei ON cce.entity_id = ccei.entity_id 
        LEFT JOIN `eav_attribute` ea ON ccev.attribute_id = ea.attribute_id 
        LEFT JOIN `eav_attribute` ea1 ON ccei.attribute_id = ea1.attribute_id 
        WHERE ea.attribute_code = 'thumbnail_image' AND ea1.attribute_code = 'is_landing' AND ccei.value = 1";
    ...
    protected function _getHtml(
        ...
        foreach ($children as $child) {
            ...
            $id = explode("-", $child->getId())[2];
            $html .= '<li ' . $this->_getRenderedMenuItemAttributes($child) . '>';
            $html .= '<a href="' . $child->getUrl() . '" ' . $outermostClassCode . '><span>';
            if($this->getImage((int) $id)) {
                $html .= '<img class="thumbnail-image-category" src="'.$this->getBaseUrl().$this->getImage((int)$id).'" alt="thumbnail" />';
            }
